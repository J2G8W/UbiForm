...
bool compareSchemaObjects(rapidjson::Value &schema1, rapidjson::Value &schema2) {
    ...
    if (!(schema1.HasMember("type") && schema1["type"].IsString() &&
          strncmp(schema1["type"].GetString(), "object", 6) == 0)) {
        return false;
    }
    if (!(schema2.HasMember("type") && schema2["type"].IsString() &&
          strncmp(schema2["type"].GetString(), "object", 6) == 0)) {
        return false;
    }
...
bool compareSchemaArrays(rapidjson::Value &schema1, rapidjson::Value &schema2) {
    ...
    if (!(schema1.HasMember("items") && schema1["items"].IsObject() && schema2.HasMember("items") &&
          schema2["items"].IsObject())) {
        return false;
    }
    ...
    if (!(items1.HasMember("type") && items1["type"].IsString() && items2.HasMember("type") &&
          items2["type"].IsString())) {
        return false;
    }
