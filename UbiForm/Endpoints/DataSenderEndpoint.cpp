...
void DataSenderEndpoint::sendMessage(SocketMessage &s) {
    if(!socketOpen){
        throw SocketOpenError("Could not send message, socket is closed", endpointType, endpointIdentifier);
    }
    int rv;
...
void DataSenderEndpoint::asyncSendMessage(SocketMessage &s) {
    if(!socketOpen){
        throw SocketOpenError("Could not async-send message, socket is closed", endpointType, endpointIdentifier);
    }
